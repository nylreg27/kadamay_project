...
class Payment(models.Model):
    ...
    date_validated = models.DateTimeField(null=True, blank=True, verbose_name="Date Validated")
    date_cancelled = models.DateTimeField(null=True, blank=True, verbose_name="Date Cancelled")
    ...
    def __str__(self):
        return f"OR {self.or_number} - {self.amount} ({self.individual.full_name if self.individual else 'N/A'})"
...
class PaymentCoveredMember(models.Model):
    ...
    individual = models.ForeignKey(Individual, on_delete=models.CASCADE, verbose_name="Member Covered")
    amount_covered = models.DecimalField(max_digits=10, decimal_places=2, default=0, verbose_name="Amount Covered")
    ...
    def __str__(self):
        return f"{self.individual.given_name if self.individual else 'N/A'} (for OR #{self.payment.or_number})"